...
本项目为游戏minecraft（中文译名《我的世界》）java版模组，游戏版本1.21.1，模组加载器为neoforge（支持版本21.1.50及以上）。
...
- 联网时进行过的所有交易会被记录在此；可在联网时与村民进行远程交易。
- 线下与线上交易独立，不相互影响，线上交易不加村民经验。
- 与不同村民个体进行相同交易，最大库存会叠加。
